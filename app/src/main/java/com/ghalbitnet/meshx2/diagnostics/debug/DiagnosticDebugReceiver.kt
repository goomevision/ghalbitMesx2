...
class DiagnosticDebugReceiver : BroadcastReceiver() {
    ...
    companion object {
        ...
        const val ACTION_RUN_OUTBOUND_CALL_TO_VIRTUAL_PEER_FULL_FLOW = "com.ghalbitnet.meshx2.debug.RUN_OUTBOUND_CALL_TO_VIRTUAL_PEER_FULL_FLOW"
        ...
        scope.launch {
            try {
                when (action) {
                    ...
                    ACTION_RUN_OUTBOUND_CALL_TO_VIRTUAL_PEER_FULL_FLOW -> {
                        Log.i("GHALBIT-DEBUG-TRIGGER", "DISPATCH target=outbound_call_virtual_peer_full_flow")
                        val result = VirtualPeerOutboundCallProbe.launchServerFirstOutgoingCallFullFlow(context.applicationContext)
                        Log.i(
                            "GHALBIT-DEBUG-TRIGGER",
                            "RESULT status=${result.status} callId=${result.callId} route=${result.routeLabel} target=${result.targetGlobalId}"
                        )
