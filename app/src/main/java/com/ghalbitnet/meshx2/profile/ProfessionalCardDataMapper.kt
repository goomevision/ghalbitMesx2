...
object ProfessionalCardDataMapper {
    ...
    fun fromProfile(context: Context?, profile: CommunityProfile): Result {
        ...
        val model = ProfessionalCardUiModel(
            ...
            careerHeadline = profile.careerHeadline?.ifBlank { "Anggota Komunitas" } ?: "Anggota Komunitas",
            visionStatement = profile.visionStatement?.ifBlank { "Belum diisi" } ?: "Belum diisi",
            missionStatement = profile.missionStatement?.ifBlank { "Belum diisi" } ?: "Belum diisi",
            activeProjects = profile.activeProjects.filter { it.isNotBlank() }.take(3),
            skillsOffered = profile.skillsOffered.filter { it.isNotBlank() }.take(5),
            skillsWanted = profile.skillsWanted.filter { it.isNotBlank() }.take(5),
            helpOffered = profile.helpOffered.filter { it.isNotBlank() }.take(3),
            helpNeeded = profile.helpNeeded.filter { it.isNotBlank() }.take(3),
            portfolioLinks = profile.portfolioLinks.filter { it.isNotBlank() }.take(3),
            communityRoles = profile.communityRoles.filter { it.isNotBlank() }.take(5),
            availabilityStatus = profile.availabilityStatus?.ifBlank { "Belum tersedia" } ?: "Belum tersedia",
            tier = tier,
