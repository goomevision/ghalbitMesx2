# fix_network_map.ps1
# Perbaiki crash NullPointerException pada Network Map
...
Write-Host "Memperbaiki NetworkActivity (crash info window)..." -ForegroundColor Cyan

$fixedNetworkActivity = @'
...
class NetworkActivity : AppCompatActivity() {
    ...
    private fun addMarkerForNode(node: MeshNode) {
        ...
        // Tidak perlu set infoWindow kustom; gunakan info window bawaan
...
Write-FileWithoutBOM "app\src\main\java\com\ghalbitnet\meshx2\monitor\NetworkActivity.kt" $fixedNetworkActivity
...
if (Test-Path "app\build\outputs\apk\debug\app-debug.apk") {
    ...
    if (Test-Path $adb) {
        ...
        Write-Host "Instalasi selesai. Buka Network Map, ketuk marker tidak akan crash." -ForegroundColor Cyan
